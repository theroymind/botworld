-- Solar layer: benchmark #2 (the GPU-instanced drone swarm) wrapped as a game
-- layer. The camera, spawn keys, perf HUD, and bodies+swarm wiring that lived
-- in main.lua moved here unchanged. The orbital sim is per-frame eye candy so
-- it only runs while active; tick() advances a placeholder counter to prove
-- backgrounded layers keep producing.
...
solar.state = { counter = 0 } -- placeholder sim output, +1 per sim second
...
local function draw_hud()
  ...
  local sim_seconds = math.floor(solar.state.counter)
  love.graphics.print(string.format("sim     %d (ticks in background)", sim_seconds), 10, 74)
  love.graphics.print(
    "[space] +10k   [b] +100k   [m] +1M   [c] clear   [wheel] zoom   [drag] pan",
    10,
    98
  )
end
...
function solar.wheelmoved(_, dy)
  if dy == 0 then
    return
  end
...
function solar.mousemoved(_, _, dx, dy)
