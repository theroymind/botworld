...
-- the current stab level + flavor and a right-pinned "build" button carrying the next
-- geometric ATP cost, enabled on the buffer. Buying lifts the safe ROS ceiling + speeds
-- ROS clearance (catalog.STAB_TOLERANCE/STAB_CLEAR), so a player watching the vitals can
-- defend a deliberate surplus instead of trimming power -- a second valid strategy.
local function stabilization_group(s)
  ...
  return {
    ...
    layout.hstack({
      layout.vstack({
        layout.text(string.format("Antioxidants   x%d", s.stab or 0), { color = colors.ui.text }),
        layout.text("clear oxidative stress, run hotter safely", { color = colors.ui.text_faint }),
      }, { gap = 2 }),
      action_button_node({
        label = "build",
        sublabel = format.number(cost) .. " atp",
...
local function build_panel(s)
  ...
  -- (so the panel numbers and the cell's flow can never disagree). STABILIZATION sits
  -- with it -- the counter-lever the vitals point a player toward when ROS climbs.
  local vitals = vitals_group(build_snapshot(s))

  local groups = {
    layout.vstack(header, { gap = theme.spacing.xs }),
    layout.vstack(power_group, { gap = theme.spacing.xs }),
    layout.vstack(vitals, { gap = theme.spacing.xs }),
