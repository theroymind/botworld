...
if arg[5] then
  catalog.FORK_AT = num(5, catalog.FORK_AT)
end
if arg[6] then
  catalog.VALUE_PER_STAGE = num(6, catalog.VALUE_PER_STAGE)
end
...
local fork_time = nil -- set to the sim time `t` when the FORK gate is first reached
...
while t < max_t do
  ...
  if curve_mode and t >= next_sample then
    ...
    io.write(
      string.format(
        "t=%5.1fm built=%-10.0f thru=%-7.1f mito=%d\n",
        t / 60,
        s.built,
        rates.throughput,
        s.mito
...
print(string.rep("-", 56))
print(
  string.format("FORK at %s  (built target %d, mito %d)", fmt(fork_time), catalog.FORK_AT, s.mito)
)
